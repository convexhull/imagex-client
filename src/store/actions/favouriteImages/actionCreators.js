    ...
    switch (platform) {
      ...
      case "cv":
        data = {
          ...
          smallImageUrl: image.assets.preview.url,
          mediumImageUrl: image.assets.preview.url,
          largeImageUrl: image.assets.preview.url,
          downloadUrl: image.assets.preview.url,
          aspect: image.aspect,
        };
        break;
      // default:
  ...
  return async (dispatch) => {
    ...
    try {
      let apiResponse = await Axios.delete(
        `/users/removeFavouriteImage?imageId=${imageId}`,
        config
      );
